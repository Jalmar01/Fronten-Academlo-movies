                ...
                <Col>
                    ...
                    <ListGroup horizontal>
                        {movie.genres?.map(genre => (
                            <ListGroup.Item key={`movies${genre.id}`}>
                                {genre.name}
                            </ListGroup.Item>
                        ))}
                    </ListGroup>
                    ...
                    <Row xs={1} sm={2} xl={3} className="g-4">
                        {movie.directors?.map((director) => (
                            <DirectorCard director={director} key={`director${director.id}`} showOptions={false} />
                        ))}
                    </Row>
                    <h3 className="mt-5">Actors</h3>
                    <Row xs={1} sm={2} xl={3} className="g-4">
                        {movie.actors?.map((actor) => (
                            <ActorCard actor={actor} key={`actor${actor.id}`} showOptions={false} />
                        ))}
                    </Row>
                </Col>
            </Row>
